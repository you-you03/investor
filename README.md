...
| 項目 | 値 |
|---|---|
| ライブ予算 | ¥200,000（約 $1,340。¥1,000,000枠は検証用シミュレーション） |
...
| ストップ/目標 | 新規ポジションでは両方必須。未設定時は新規BUYを停止 |
...
2. Claude がスクリーニング → ファンダ60%・中期RS25%・検証済み材料15%でスコアリング
3. スコア ≥ 7.5 の候補を `data/research_history.json` に保存
